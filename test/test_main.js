...
am(
  typeof JsonRisk.period_str_to_time === "function",
  "period_str_to_time function defined",
);
am(
  typeof JsonRisk.get_safe_date === "function",
  "get_safe_date function defined",
);
am(
  typeof JsonRisk.date_str_to_date === "function",
  "date_string_to_date function defined",
);
am(typeof JsonRisk.get_rate === "function", "get_rate function defined");
am(typeof JsonRisk.get_df === "function", "get_df function defined");
am(
  typeof JsonRisk.get_fwd_amount === "function",
  "get_fwd_amount function defined",
);
am(
  typeof JsonRisk.get_const_curve === "function",
  "get_const_curve function defined",
);
am(
  typeof JsonRisk.get_safe_curve === "function",
  "get_safe_curve function defined",
);
...
for (i = 1; i < 11; i++) {
  am(
    JsonRisk.add_months(from, i * i).getTime() ===
      get_utc_date(2000, 1 + i * i, 25).getTime(),
    "Month addition (pos)",
  );
  am(
    JsonRisk.add_months(from, -i * i).getTime() ===
...
am(
  JsonRisk.date_str_to_date("28.2.2018").getTime() ===
    get_utc_date(2018, 1, 28).getTime(),
...
c = { type: "yield", labels: ["1Y", "7Y", "12Y", "186M", "20Y"], dfs: c.dfs };
...
var c2 = { type: "yield", labels: ["5Y", "10Y"], zcs: [0.02, 0.02] };
...
for (i = 2; i <= 12; i++) {
  ...
  am(
    same_dates(sched, expected),
    "Schedule generation consistency short stub at beginning (" + (i - 1) + ")",
  );
  ...
  am(
    same_dates(sched, expected),
    "Schedule generation consistency long stub at beginning (" + (i - 1) + ")",
  );
...
for (i = 0; i < 400; i++) {
  ...
  am(
    p1.toFixed(3) === "100.000",
    "Fair spread derivation for amortising float rate bonds (" + (i + 1) + ").",
  );
}
...
am(
  check(results),
  "Vector pricing with floater returns valid vector of numbers",
);
...
am(
  check(results),
  "Vector pricing with fxterm returns valid vector of numbers (0)",
);
...
am(
  check(results),
  "Vector pricing with fxterm returns valid vector of numbers (1)",
);
...
am(
  check(results),
  "Vector pricing with amortizing bond returns valid vector of numbers",
);
...
am(
  check(results),
  "Vector pricing with swaption returns valid vector of numbers",
);
...
am(
  check(results),
  "Vector pricing with callable bond returns valid vector of numbers",
);
...
am(
  compare(results_vector, results_scen_rf),
  "Vector pricing with scenarios by risk factor, with FX",
);
...
am(
  compare(results_vector, results_scen_rf),
  "Vector pricing with scenarios by risk factor, with FX and equity",
);
...
values = [
  [0.149, 0.134, 0.1228, 0.1189, 0.1148],
  [0.129, 0.1201, 0.1146, 0.1108, 0.104],
  [0.1149, 0.1112, 0.107, 0.101, 0.0957],
  [0.1047, 0.1021, 0.098, 0.0951, 0.127],
  [0.1, 0.095, 0.09, 0.123, 0.116],
];
...
for (i = 0; i < 13; i++) {
  ...
  result_multi = JsonRisk.pricer_callable_bond(
    bond,
    curve,
    null,
    curve,
