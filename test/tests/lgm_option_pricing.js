...
test.execute = function (TestFramework, JsonRisk) {
    ...
    var cf_obj = {
        date_pmt: [TestFramework.get_utc_date(2019, 0, 1),
    ...
    expiries = [TestFramework.get_utc_date(2019, 0, 1),
        TestFramework.get_utc_date(2020, 0, 1),
        TestFramework.get_utc_date(2021, 0, 1),
        TestFramework.get_utc_date(2022, 0, 1),
        TestFramework.get_utc_date(2023, 0, 1),
    ...
    for (i = 0; i < expiries.length; i++) {
        ...
        TestFramework.assert(Math.abs(result - result_orig) / bpv < 1, "LGM option price (equivalent regular vs original), first_exercise_date " + (i + 1));
        ...
        TestFramework.assert(Math.abs(result - result_orig) / bpv < 1, "LGM option price (numeric vs original), first_exercise_date " + (i + 1));
        ...
        TestFramework.assert(Math.abs(result - result_orig) / bpv < 1, "LGM option price curve up (equivalent regular vs original), first_exercise_date " + (i + 1));
        result = JsonRisk.lgm_bermudan_call_on_cf(cf_obj, [yf(JsonRisk.valuation_date, expiries[i])], curve_100bp, [lgm_xi]);
        TestFramework.assert(Math.abs(result - result_orig) / bpv < 1, "LGM option price curve up (numeric vs original), first_exercise_date " + (i + 1));
