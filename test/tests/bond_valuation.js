...
test.execute = function (TestFramework, JsonRisk) {
    ...
    TestFramework.assert("105.0" === JsonRisk.pricer_bond(bond, curve, null, null).toFixed(1), "bond valuation (1)");

    bond.settlement_days = 1;

    TestFramework.assert("100.0" === JsonRisk.pricer_bond(bond, curve, null, null).toFixed(1), "bond valuation (2)");

    bond.tenor = 6;
    TestFramework.assert("100.5" === JsonRisk.pricer_bond(bond, curve, null, null).toFixed(1), "bond valuation (3)");

    bond.tenor = 3;
    TestFramework.assert("100.7" === JsonRisk.pricer_bond(bond, curve, null, null).toFixed(1), "bond valuation (4)");
    ...
    for (i = 0; i < Kupon.length; i++) {
        ...
        TestFramework.assert(pu < Kurs_Dirty[i] && Kurs_Dirty[i] < pd, "Bond Valuation (Real BUND Bonds using yield curve, " + (i + 1) + ")");
    }
    ...
    for (i = 0; i < Kupon.length; i++) {
        ...
        TestFramework.assert(pu < Kurs_Dirty[i] && Kurs_Dirty[i] < pd, "Bond Valuation (Real BUND Bonds using spread curve, " + (i + 1) + ")");
    }
    ...
    for (i = 0; i < Kupon.length; i++) {
        ...
        TestFramework.assert(pu < Kurs_Dirty[i] && Kurs_Dirty[i] < pd, "Bond Valuation (Real BUND Bonds at interest payment date minus settlement days, " + (i + 1) + ")");
    }
    ...
    for (i = 0; i < Kupon.length; i++) {
        ...
        TestFramework.assert(pu < Kurs_Dirty[i] && Kurs_Dirty[i] < pd, "Bond Valuation (Real BUND Bonds just before interest payment date minus settlement days, " + (i + 1) + ")");
    }
    ...
    for (i = 0; i < Kupon.length; i++) {
        ...
        TestFramework.assert(pv.toFixed(8) === pv_ref.toFixed(8), "Bond Valuation against reference price, with unadjusted accrual periods (" + (i + 1) + ")");
        ...
        TestFramework.assert(pv.toFixed(8) === pv_ref.toFixed(8), "Bond Valuation against reference price, with adjusted accrual periods (" + (i + 1) + ")");
    }
    ...
    for (i = 0; i < Kupon.length; i++) {
        ...
        TestFramework.assert(pu < Kurs_Dirty[i] && Kurs_Dirty[i] < pd, "Floater Valuation (using constant forward curve with rate 0.0 and a positive float_spread, " + (i + 1) + ")");
    }
    ...
    for (i = 0; i < Kupon.length; i++) {
        ...
        console.log("Quote from www.bundesbank.de:                   " + Kurs_Dirty[i].toFixed(3));
        console.log("JSON Risk Price one basis point more expensive: " + pd.toFixed(3));
        TestFramework.assert(pu < Kurs_Dirty[i] && Kurs_Dirty[i] < pd, "Floater Valuation (using constant forward curve with rate reflecting Bund coupon and a zero float_spread, " + (i + 1) + ")");
    }
    ...
    for (i = 0; i < months.length; i++) {
        for (j = 0; j < tenors.length; j++) {
            ...
            TestFramework.assert(p_diff < months[i] / 12, "Swap Valuation (" + ((i) * tenors.length + (j + 1)) + ")");
        }
    }
