...
test.execute = function (TestFramework, JsonRisk) {
    ...
    surface = {
        ...
        expiries: [1, 2, 5],
        terms: [1, 5, 10, 20],
        values: [[0.008, 0.0081, 0.0082, 0.0080], [0.0085, 0.0076, 0.0068, 0.0068], [0.01, 0.0103, 0.012, 0.011]]
    };
    ...
    for (i = 0; i < Maturity.length; i++) {
        ...
        TestFramework.assert(Math.abs((result_european - result_swaption) / bpv) < 1, "Callable bond consistency check (" + (i + 1) + ")");
        TestFramework.assert(result_european >= result_bermudan, "Multi-callable bond consistency check (" + (i + 1) + ")");
    }
