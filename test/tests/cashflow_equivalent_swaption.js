...
test.execute = function (TestFramework, JsonRisk) {
    ...
    var surface = {
        ...
        expiries: [1, 2, 3],
        terms: [1, 2, 3, 4],
        values: [[0.01, 0.01, 0.02, 0.02], [0.02, 0.02, 0.03, 0.03], [0.03, 0.03, 0.04, 0.04]]
    };
    for (i = 0; i < months.length; i++) {
        for (j = 0; j < expiries.length; j++) {
            ...
            TestFramework.assert(swaption.maturity.getTime() === bond.maturity.getTime(), "Test equivalent swaption consistency for bullet bonds (maturity)");
            TestFramework.assert(swaption.fixed_rate.toFixed(5) === bond.fixed_rate.toFixed(5), "Test equivalent swaption consistency for bullet bonds (rate)");
            console.log("-----------------");
        }
    }
