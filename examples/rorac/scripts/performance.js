...
var update_scenarios = function (timeindex) {
  ...
  for (k = 0; k < curves.zcs_scen.length; k++) {
    ...
    for (j = 0; j < z[0].length; j++) {
      z[k + 1][j] =
        curves.zcs_hist[timeindex][j] + Math.sqrt(12) * curves.zcs_scen[k][j];
    }
  }
};
...
for (i = 0; i < curves.dates.length - 1; i++) {
  ...
  for (x in tmp) {
    res[i][x] = {
      ...
      mean: mean(tmp[x]),
      risk:
        ((-(quantile(tmp[x], 0.01) - mean(tmp[x])) * Math.sqrt(1)) / 2.32) *
        3.09,
    };
  }

  yield_riskless = jr.get_rate(
    jr.get_safe_curve({
      labels: params.curves.ZINSKURVE.labels,
      zcs: params.curves.ZINSKURVE.zcs[0],
    }),
    1 / 365,
  );
  ...
  for (x in tmp) {
    ...
    res[i][x].yield =
      ((res[i][x].pv_end - res[i][x].pv_start) / res[i][x].pv_start) * 12;
    ...
    res[i][x].leveraged_pnl =
      res[i][x].amount * res[i][x].yield +
      res[i][x].amount_riskless * yield_riskless;
    res[i][x].leveraged_pnl_excess =
      res[i][x].leveraged_pnl - equity * yield_riskless;
    res[i][x].leveraged_yield = res[i][x].leveraged_pnl / equity;
    res[i][x].leveraged_yield_excess =
      res[i][x].leveraged_yield - yield_riskless;
