...
app.controller("main_ctrl", [
  ...
  function ($scope) {
    ...
    function update_data() {
      var index_value = $scope.rorac.values_names.indexOf(
        $scope.value_names.selection,
      );
    ...
    function update_table(index_value) {
      ...
      for (i = 0; i < $scope.rorac.dates.length; i++) {
        ...
        for (k = 0; k < $scope.rorac.subportfolio.length; k++) {
          data_table[i][$scope.rorac.subportfolio[k]] =
            $scope.rorac.values[k][index_value][i];
        }
      }
      // fill columns
      $(function () {
