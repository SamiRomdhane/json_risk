    ...
    if (req.status === 200) {
      ...
      for (j = 1; j < keys.length; j++) {
        ...
        if (key === "curves" || key === "scalars" || key === "surfaces") {
          sc.params_count =
            sc.params_count + (Object.keys(sc.params[key]).length || 0);
        }
      }
      sc.$apply();
    } else {
  ...
  if (kind === "params") {
    ...
    pp_config.complete = function (results, file) {
      ...
      if (!sc.params.scenario_names) {
        ...
        if (scenarios.length > sc.params.scenario_names)
          sc.params.scenario_names = scenarios;
      }
    ...
    pp_config.complete = function (results, file) {
      ...
      if (
        Math.round(tmpexpiry.length / labels_expiry.length) !=
        tmpexpiry.length / labels_expiry.length
      ) {
        sc.upload_error =
          "Could not upload surfaces " +
          results.data[0][0] +
          ": please check columns and rows.";
        sc.$apply();
        return;
      }
      ...
      for (n = 0; n < labels_expiry.length; n++) {
        if (
          tmpexpiry.filter((x) => x === labels_expiry[0]).length !=
          tmpexpiry.filter((x) => x === labels_expiry[n]).length
        ) {
          // Errorhandling
          sc.upload_error =
            "Could not upload surfaces " +
            results.data[0][0] +
            ": different sceanrio count for expiries.";
          sc.$apply();
          return;
        }
        var k = 0;
        for (i = 1; i < results.data.length; i++) {
          ...
          if (results.data[i].length !== results.data[0].length) {
            sc.upload_error =
              "Could not upload surfaces " +
              results.data[0][0] +
              ": different term count for expiries.";
            sc.$apply();
            return;
          }
