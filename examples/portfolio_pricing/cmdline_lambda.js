...
if (!JsonRisk.get_safe_date(params.valuation_date)) {
  console.error(
    "ERROR: Invalid parameter file, should at least contain an object valid valuation_date entry.",
  );
  process.exit(1);
}
...
if (!Array.isArray(portfolio)) {
  console.error(
    "ERROR: Invalid portfolio file, should at least contain an array.",
  );
  process.exit(1);
}
...
var add_error = function (obj) {
  ...
  while (j < output.errors.length) {
    if (output.errors[j].msg === obj.msg) {
...
var add_warning = function (obj) {
  ...
  while (j < output.warnings.length) {
    if (output.warnings[j].msg === obj.msg) {
...
var handle_response = function (data, chunk) {
  ...
  if (
    ...
    console.error(
      `Request with ${chunk.length} instruments does not return any valid data`,
    );
    add_error({
    ...
    console.error(
      `INFO: Response with ${chunk.length} instruments received, ${portfolio.length} instruments remaining to send, ${remaining} instruments remaining to receive, ${running} requests currently running`,
    );
    ...
    output.times["calc_max"] =
      (output.times["calc_end"] - output.times["calc_start"]) / 1000;
  ...
  if (0 === remaining) {
    ...
    console.error(
      "INFO: Done. Valuations took " + (t1 - t0) / 1000 + " seconds.",
    );
...
var new_request_with_chunk = function (chunk, num_retries) {
  ...
  console.error(
    `INFO: Request  with ${chunk.length} instruments sent,     ${portfolio.length} instruments remaining to send, ${remaining} instruments remaining to receive, ${running} requests currently running`,
  );
...
output.times["calc"] = 0.0;
output.times["params_max"] = 0;
output.times["params"] = 0.0;
var tmp = [];
...
let req = https.request(options_jrparams, (res) => {
  ...
  res.on("end", () => {
    ...
    if (data.path) {
      ...
      console.error(
        "ERROR: Could not upload params, jrparams returns: " + data.message,
      );
      process.exit(1);
    }
  });
