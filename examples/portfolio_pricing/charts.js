...
var chart_scenario_subportfolio = new Chart(
  document.getElementById("canvas-scenario-subportfolio"),
  {
    type: "bar",
    data: {
      labels: [""],
...
var update_chart_scenarios = function (
  result,
  subportfolio,
  type,
  scenario_names,
  ...
  if (subportfolio != null)
    chart_scenarios.data.datasets[1].label = subportfolio;
  if (subportfolio === null)
    chart_scenarios.data.datasets[1].label = "Subportfolio";
  var keys = Object.keys(result);
  for (var i = 0; i < keys.length; i++) {
    ...
    if (type === "pnl") {
      chart_scenarios.data.datasets[0].data[i] =
        result[key]["TOTAL"] - result[0]["TOTAL"];
      chart_scenarios.data.datasets[1].data[i] =
        result[key][subportfolio] - result[0][subportfolio];
