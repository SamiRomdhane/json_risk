...
assign_disc_curve = function (i) {
  ...
  if (cname in p.curves) {
    ...
    if (i.disc_curve)
      msg += " Originally assigned curve was not found in parameters.";
    i.disc_curve = cname;
...
assign_surface = function (i) {
  ...
  if (cname in p.surfaces) {
    ...
    if (i.surface)
      msg += " Originally assigned surface was not found in parameters.";
    i.surface = cname;
    add_warning(msg, i.id || "");
  } else {
...
var add_error = function (msg, id) {
  ...
  if (j >= output.errors.length)
    output.errors.push({ msg: msg, id: id, count: 1 }); //new error
};

var add_warning = function (msg, id) {
  var j = 0;
  while (j < output.warnings.length) {
    if (output.warnings[j].msg === msg) {
  ...
  if (j >= output.warnings.length)
    output.warnings.push({ msg: msg, id: id, count: 1 }); //new warning
};
...
for (j = 0; j < portfolio.length; j++) {
  ...
  if (0 === j % 100 || portfolio.length - j < 100)
    console.error(
      `INFO: ${j} instruments priced, ${portfolio.length - j} instruments remaining`,
    );
}
