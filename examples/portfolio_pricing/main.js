...
app.controller("main_ctrl", [
  ...
  function ($scope) {
    ...
    $scope.warnings = null;
    $scope.options_aws = {
      // xmlhttprequest options for aws (amazon web services)
    ...
    $scope.load_test_params = function () {
      ...
      $scope.params.curves["EUR_1M_FWD"] =
        $scope.params.curves["EUR_OIS_DISCOUNT"];
      $scope.params.curves["EUR_3M_FWD"] =
        $scope.params.curves["EUR_OIS_DISCOUNT"];
      $scope.params.curves["EUR_6M_FWD"] =
        $scope.params.curves["EUR_OIS_DISCOUNT"];
      $scope.params.curves["USD_OIS_DISCOUNT"] =
        $scope.params.curves["EUR_OIS_DISCOUNT"];
      $scope.params.curves["USD_1M_FWD"] =
        $scope.params.curves["EUR_OIS_DISCOUNT"];
      $scope.params.curves["USD_3M_FWD"] =
        $scope.params.curves["EUR_OIS_DISCOUNT"];
      $scope.params.curves["USD_6M_FWD"] =
        $scope.params.curves["EUR_OIS_DISCOUNT"];
      $scope.res = null;
      $scope.errors = null;
      $scope.warnings = null;
      $scope.params_count =
        (Object.keys($scope.params.curves).length || 0) +
        (Object.keys($scope.params.scalars).length || 0) +
        (Object.keys($scope.params.surfaces).length || 0);
    };
    ...
    $scope.count_scenarios_curve = function (curve) {
      return Array.isArray((curve.dfs || curve.zcs)[0])
        ? (curve.dfs || curve.zcs).length
        : 1;
    };
    ...
    $scope.remove_parameter = function (key, value, kind) {
      if (kind === "scalars") {
        ...
        if (Object.keys($scope.params.scalars).length === 0)
          delete $scope.params.scalars;
      }
      if (kind === "curves") {
        delete $scope.params.curves[key];
        if (Object.keys($scope.params.curves).length === 0)
          delete $scope.params.curves;
      }
      if (kind === "surfaces") {
        delete $scope.params.surfaces[key];
        if (Object.keys($scope.params.surfaces).length === 0)
          delete $scope.params.surfaces;
      }
      if (kind === "calendars") {
        delete $scope.params.calendars[key];
        if (Object.keys($scope.params.calendars).length === 0)
          delete $scope.params.calendars;
      }
      $scope.res = null;
      $scope.errors = null;
      $scope.warnings = null;
    ...
    $scope.load_scenarios = function (type) {
      update_chart_scenarios(
        $scope.res,
        $scope.available_subportfolios.selection,
        type,
        $scope.params.scenario_names,
      ); // function in charts.js
    ...
    $scope.add_error = function (obj) {
      ...
      while (j < $scope.errors.length) {
        if ($scope.errors[j].msg === obj.msg) {
          //same error has already occured
    ...
    $scope.add_warning = function (obj) {
      ...
      while (j < $scope.warnings.length) {
        if ($scope.warnings[j].msg === obj.msg) {
          //same warning has already occured
    ...
    $scope.calculate = function () {
      ...
      for (i = 0; i < $scope.conc; i++) {
        ...
        wrk[i].onmessage = function (e) {
          if (e.data.res) {
            ...
            if (!$scope.res) {
              ...
              for (j = 0; j < e.data.res.length; j++) {
                //make object structure allowing to distinguish subportfolios
                $scope.res[j] = {};
              ...
              //result to be added
              for (j = 0; j < e.data.res.length; j++) {
                $scope.res[j][sub_portfolio] =
                  ($scope.res[j][sub_portfolio] || 0) + e.data.res[j];
          ...
          if (0 === incomplete) {
            //all done, terminate workers and exit
            while (wrk.length) {
              wrk[0].terminate();
              wrk.shift();
            }
          ...
          if (incomplete % 100 === 0) {
            //every now and then, update display and stats
            t1 = new Date().getTime();
          ...
          if (unsent > 0) {
            // queue next instrument while not done
        ...
        wrk[i].onerror = function (e) {
          $scope.add_error({
            msg:
              "An error occurred in the worker process. " + (e.message || ""),
            id: "unknown",
            count: 1,
          });
        };
    ...
    $scope.calculate_lambda = function () {
      ...
      const options_aws_header = {
        // header aws request
        reqheader: {
          0: ["x-api-key", $scope.options_aws.apikey],
          1: ["x-amz-docs-region", "eu-central-1"],
          2: ["content-type", "application/json"],
          3: ["Content-Encoding", "gzip"],
        }, // '4': ['Accept-Encoding', 'gzip,identity']},  // header
        reqheader_count: "4", // number of header
      };

      const options_jrparams = {
        // xmlhttprequest options for uploading params to jsonrisk.de/jrparams/
      ...
      var handle_response_lambda = function (data, chunk) {
        ...
        if (
          ...
          undefined === data.warnings ||
          undefined === data.times
        ) {
          $scope.add_error({
            msg: `Request with ${chunk.length} instruments does not return any valid data`,
            id: chunk[0].id || "",
            count: chunk.length,
          });
        } else {
          console.log(
            `INFO: Response with ${chunk.length} instruments received, ${portfolio_in.length} instruments remaining to send, ${remaining} instruments remaining to receive, ${running}   requests currently running`,
          );
          //$scope.$apply();
          ...
          for (var k = 0; k < keys.length; k++) {
            ...
            for (var j = 0; j < data.res[sub_port].length; j++) {
              ...
              $scope.res[j][sub_port] =
                ($scope.res[j][sub_port] || 0) + data.res[sub_port][j];
              if (undefined === $scope.res[j]["TOTAL"])
                $scope.res[j]["TOTAL"] = 0;
              $scope.res[j]["TOTAL"] =
                $scope.res[j]["TOTAL"] + data.res[sub_port][j];
            }
          }

          // aggregate times (only for analysis)
          aws_analysis.times["calc"] =
            aws_analysis.times["calc"] + data.times["calc"];
          aws_analysis.times["exec"] =
            aws_analysis.times["exec"] + data.times["exec"];

          // aggregate errors and warnings
          $scope.analytics.error_count =
            $scope.analytics.error_count + data.error_count;
          $scope.analytics.warning_count =
            $scope.analytics.warning_count + data.warning_count;
          ...
          // warnings and errors
          for (n = 0; n < data.warnings.length; n++)
            $scope.add_warning(data.warnings[n]);
          for (n = 0; n < data.errors.length; n++)
            $scope.add_error(data.errors[n]);
      ...
      var new_request_with_chunk_lambda = function (chunk, num_retries) {
        running++;
        console.log(
          `INFO: Request  with ${chunk.length} instruments sent, ${portfolio_in.length} instruments remaining to send, ${remaining} instruments remaining to receive, ${running} requests currently running`,
        );

        var data = "";
        var xmlhttp_aws = new XMLHttpRequest();
        xmlhttp_aws.open(
          $scope.options_aws.method,
          $scope.options_aws.hostname + $scope.options_aws.path,
        );
        for (m = 0; m < options_aws_header.reqheader_count; m++)
          xmlhttp_aws.setRequestHeader(
            options_aws_header.reqheader[m][0],
            options_aws_header.reqheader[m][1],
          ); //Header setzen
        ...
        xmlhttp_aws.onerror = function () {
          ...
          if (num_retries < config.max_retries) {
            ...
            $scope.add_error({
              msg: `ERROR: Problem with request: ${e.message} , max retries reached.`,
              id: chunk[0].id || "",
              count: chunk.length,
            });
            remaining -= chunk.length;
        ...
        xmlhttp_aws.send(
          pako.gzip(
            JSON.stringify(
              { paramsurl: { url: paramsurl }, portfolio: chunk },
              repl,
            ),
            { level: 9 },
          ),
      ...
      $scope.add_warning({
        msg: "INFO: Start: " + new Date(new Date().toUTCString()),
      });
      ...
      xmlhttp_aws_test.open(
        $scope.options_aws.method_test,
        $scope.options_aws.hostname + $scope.options_aws.path,
      );
      for (m = 0; m < options_aws_header.reqheader_count; m++)
        xmlhttp_aws_test.setRequestHeader(
          options_aws_header.reqheader[m][0],
          options_aws_header.reqheader[m][1],
        ); //Header setzen
      ...
      xmlhttp_aws_test.addEventListener("load", function (evt) {
        if (xmlhttp_aws_test.status === 200) {
          ...
          var params_blob = new Blob([JSON.stringify($scope.params)], {
          ...
          xmlhttp_params.addEventListener("load", function (evt) {
            if (xmlhttp_params.status === 200) {
              //response status 200
              var data = JSON.parse(xmlhttp_params.responseText);
              if (data.path) {
                paramsurl =
                  "https://" +
                  options_jrparams.hostname +
                  options_jrparams.path +
                  data.path;
                console.log("INFO: params made available under " + paramsurl);
                while (
                  portfolio_in.length > 0 &&
                  running < 2 * config.max_requests
                ) {
                  new_request_lambda();
                }
              } else {
                // error handling analog cmdline_lambda
                console.error(
                  "ERROR: Could not upload params, jrparams returns: " +
                    data.message,
                );
                process.exit(1);
              }
              ...
              console.log(
                "Error: an error occurred, but the error message could not be retrieved.",
              );
            } else {
          ...
          xmlhttp_params.open(
            options_jrparams.method,
            "https://" + options_jrparams.hostname + options_jrparams.path,
          );
          xmlhttp_params.send(form_params);
        }

        if (xmlhttp_aws_test.status === 403) {
          //response status 403 (invalid apikey)
          if (JSON.parse(this.responseText).type === "invalid apikey") {
            $scope.add_error({
              msg: "Please use a valid apikey.",
              id: "",
              count: 1,
            });
            $scope.$apply();
