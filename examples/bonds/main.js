...
app.controller("main_ctrl", [
  ...
  function ($scope) {
    $scope.instrument = {
      ...
      rate: 0.0275,
      get fixed_rate() {
        return this.rate_type === "fix" ? this.rate : null;
      },
      ...
      call_tenor: "-1",
      get type() {
        return this.call_tenor !== "-1" ? "callable_bond" : "bond";
      },
    ...
    function update() {
      JsonRisk.valuation_date = JsonRisk.get_safe_date(
        $scope.params.valuation_date,
      );
      $scope.errors = [];
      ...
      if (!$scope.params.curves) {
        $scope.errors.push(
          "no curves available, must load valid parameter set",
        );
        return;
      }
      ...
      rc =
        $scope.params.curves[$scope.instrument.refinancing_spread_curve] ||
        null;
      if (!dc) {
        $scope.errors.push(
          "discount curve not available, must load valid parameter set",
        );
        return;
      }
      ...
      if (!fc) {
        $scope.errors.push(
          "forward curve not available, must load valid parameter set",
        );
        return;
      }
      ...
      try {
        if ($scope.instrument.type === "bond") {
          ...
          if (!su) {
            $scope.errors.push(
              "volatility surface not available, must load valid parameter set",
            );
            return;
          }
        }
        ...
        $scope.res.ttm = JsonRisk.time_from_now(
          $scope.cashflows.date_pmt[$scope.cashflows.date_pmt.length - 1],
        );
      } catch (ex) {
      ...
      try {
        ...
        $scope.res.bpv_ir =
          jrinst.present_value(dc_up, sc, fc_up, su) -
          jrinst.present_value(dc_down, sc, fc_down, su);
        $scope.res.bpv_spr =
          jrinst.present_value(dc_up, sc, fc, su) -
          jrinst.present_value(dc_down, sc, fc, su);
        $scope.res.bpv_ir_opt =
          $scope.res.bpv_ir -
          base.present_value(dc_up, sc, fc_up) +
          base.present_value(dc_down, sc, fc_down);
        $scope.res.bpv_spr_opt =
          $scope.res.bpv_spr -
          base.present_value(dc_up, sc, fc) +
          base.present_value(dc_down, sc, fc);
        ...
        var overnight = new JsonRisk.fixed_income({
          ...
          maturity: JsonRisk.add_days(
            JsonRisk.get_safe_date($scope.params.valuation_date),
            1,
          ),
          tenor: 1,
          fixed_rate: 0.0,
        ...
        var current_rate_or_spread =
          $scope.instrument.rate_type === "fix"
            ? base.fixed_rate
            : base.float_spread;
        var i = 0;
        while (base.conditions_valid_until[i] < $scope.params.valuation_date)
          i++;
      ...
      //bcbs 368 scenarios
      var bcbs368times = [
        0.0028, 0.0417, 0.1667, 0.375, 0.625, 0.875, 1.25, 1.75, 2.5, 3.5, 4.5,
      ...
      try {
        ...
        $scope.res.pv_shortup = jrinst.present_value(
          dc_shortup,
          sc,
          fc_shortup,
          su,
        );
        $scope.res.pv_shortdown = jrinst.present_value(
          dc_shortdown,
          sc,
          fc_shortdown,
          su,
        );
        $scope.res.pv_steepener = jrinst.present_value(
          dc_steepener,
          sc,
          fc_steepener,
          su,
        );
        $scope.res.pv_flattener = jrinst.present_value(
          dc_flattener,
          sc,
          fc_flattener,
          su,
    ...
    $scope.import_file = function (kind, url) {
      ...
      f.addEventListener(
        "change",
        function (evt) {
          import_data(evt.target.files[0], kind, $scope);
        },
        false,
      );
      f.click();
    ...
    $scope.export_curve = function (curve_name) {
      export_curve_to_csv(
        $scope.params.curves[curve_name],
        curve_name,
        curve_name + ".csv",
      );
    };
    ...
    var unwatch_selected_params = $scope.$watch(
      "available_params.selection",
      function () {
