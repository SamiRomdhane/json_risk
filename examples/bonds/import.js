var load_params_from_server = function (sc) {
    ...
    sc.params = {
      ...
      curves: {
        EUR_ESTR: {
          ...
          zcs: [
            -0.00795291, -0.0077878800000000005, -0.00699913, -0.00569703,
            -0.00413626, -0.00099492, 0.00268452, 0.00620335, 0.00803145,
            0.00912026,
          ],
        },
        EUR_3M: {
          ...
          zcs: [
            -0.00695291, -0.0067878800000000005, -0.00599913, -0.00469703,
            -0.00313626, -0.00009492, 0.00368452, 0.00720335, 0.00903145,
            0.01012026,
          ],
        },
        EUR_6M: {
          ...
          zcs: [
            -0.00595291, -0.0057878800000000005, -0.00499913, -0.00369703,
            -0.00213626, -0.00000492, 0.00468452, 0.00820335, 0.01003145,
            0.01112026,
          ],
        },
      },
  ...
  req.open(
    "GET",
    "https://www.jsonrisk.de/jrparams/" + sc.available_params.selection,
  );
  req.send();
};
  ...
  req.addEventListener("load", function (evt) {
    if (req.status === 200) {
      ...
      for (var i = 0; i < result.length; i++) {
        if (result[i].toLowerCase().includes("base"))
          sc.available_params.list.push(result[i]);
      }
...
var import_data = function (fil, kind, sc) {
  ...
  if ("string" === typeof fil) {
    ...
    (pp_config.download = false), Papa.parse(fil, pp_config);
  }
};
