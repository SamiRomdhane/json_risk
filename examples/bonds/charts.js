...
var update_chart_curves = function (dc, fc) {
  chart_curves.data.labels = JsonRisk.get_curve_times(
    JsonRisk.add_curves(dc, fc),
  ); // get common set of times
  var times = JsonRisk.get_curve_times(JsonRisk.add_curves(dc, fc)); // get common set of times
  chart_curves.data.datasets[0].data = new Array(
    chart_curves.data.labels.length,
  );
  chart_curves.data.datasets[1].data = new Array(
    chart_curves.data.labels.length,
  );
  for (var i = 0; i < chart_curves.data.labels.length; i++) {
    chart_curves.data.datasets[0].data[i] = {
      x: times[i],
      y: JsonRisk.get_rate(dc, chart_curves.data.labels[i]),
    };
    chart_curves.data.datasets[1].data[i] = {
      x: times[i],
      y: JsonRisk.get_rate(fc, chart_curves.data.labels[i]),
    };
