...
(function (library) {
  library.callable_fixed_income = function (instrument) {
    ...
    this.opportunity_spread =
      typeof instrument.opportunity_spread === "number"
        ? instrument.opportunity_spread
        : 0;
  ...
  library.callable_fixed_income.prototype.present_value = function (
    ...
    var xi_vec = library.lgm_calibrate(
      this.basket,
      disc_curve,
      fwd_curve,
      surface,
...
(function (library) {
  ...
  library.get_df = function (curve, t, imin, imax) {
    ...
    if (undefined === imax)
      imax =
        (curve.times || curve.days || curve.dates || curve.labels).length - 1;
    ...
    if (imin === imax)
      return t === get_time_at(curve, imin)
        ? get_df_at(curve, imin)
        : Math.pow(get_df_at(curve, imin), t / get_time_at(curve, imin));
    //extrapolation (constant on zero coupon rates)
    if (t < get_time_at(curve, imin))
      return Math.pow(get_df_at(curve, imin), t / get_time_at(curve, imin));
    if (t > get_time_at(curve, imax))
      return Math.pow(get_df_at(curve, imax), t / get_time_at(curve, imax));
    //interpolation (linear on discount factors)
    if (imin + 1 === imax) {
      if (get_time_at(curve, imax) - get_time_at(curve, imin) < 1 / 512)
        throw new Error(
          "get_df_internal: invalid curve, support points must be increasing and differ at least one day",
        );
      return (
        (get_df_at(curve, imin) * (get_time_at(curve, imax) - t)) /
          (get_time_at(curve, imax) - get_time_at(curve, imin)) +
        (get_df_at(curve, imax) * (t - get_time_at(curve, imin))) /
          (get_time_at(curve, imax) - get_time_at(curve, imin))
      );
    }
    //binary search and recursion
    imed = Math.ceil((imin + imax) / 2.0);
    if (t > get_time_at(curve, imed))
      return library.get_df(curve, t, imed, imax);
  ...
  library.get_fwd_rate = function (curve, tstart, tend) {
    ...
    return (
      Math.pow(
        library.get_df(curve, tend) / library.get_df(curve, tstart),
        -1 / (tend - tstart),
      ) - 1
    );
  };
...
(function (library) {
  library.fixed_income = function (instrument) {
    ...
    this.notional_exchange =
      instrument.notional_exchange === false ? false : true;
    ...
    this.residual_spread =
      typeof instrument.residual_spread == "number"
        ? instrument.residual_spread
        : 0;
    ...
    if (linear_amortization)
      this.repay_amount = [this.notional / (this.repay_schedule.length - 1)];
  ...
  library.fixed_income.prototype.finalize_cash_flows = function (
    ...
    for (i = 1; i < n; i++) {
      ...
      interest_current_period[i] =
        current_principal[i] * current_rate * c.accrual_factor[i];
...
(function (library) {
  ...
  function get_discount_factors(
    ...
    while (i < cf_obj.t_pmt.length) {
      ...
      if (residual_spread)
        df *= Math.pow(1 + residual_spread, -cf_obj.t_pmt[i]);
      res[i] = df;
      i++;
    }
    ...
    while (i < cf_obj.t_pmt.length) {
      ...
      for (j = 0; j < state.length; j++) {
        res[j] +=
          cf_obj.pmt_total[i] *
          discount_factors[i] *
          Math.exp(-dh * state[j] - dh * dh * xi * 0.5);
      }
      i++;
    }
    return res;
  ...
  library.lgm_european_call_on_cf = function (
    ...
    var res =
      -(cf_obj.current_principal[i] + accrued_interest + sadj) *
      discount_factors[discount_factors.length - 1] *
      library.cndf(break_even * one_std_dev);
  ...
  library.lgm_calibrate = function (basket, disc_curve, fwd_curve, surface) {
    ...
    for (i = 0; i < basket.length; i++) {
      if (library.time_from_now(basket[i].first_exercise_date) > 1 / 512) {
        ...
        std_dev_bachelier =
          library.get_surface_rate(surface, tte, ttm - tte) * Math.sqrt(tte);
        //initial guess
        xi = Math.pow(
  ...
  library.lgm_bermudan_call_on_cf = function (
    ...
    function numeric_integration(j) {
      //simple implementation of lgm martingale formula
      ...
      for (i = 0; i < n; i++) {
        dp_hi =
          i === n - 1
            ? 1
            : library.fast_cndf(
                (state_last[i] - state[j] + 0.5 * ds) * norm_scale,
              );
...
(function (library) {
  ...
  library.schedule = function (
    ...
    if (
      (eff_dt instanceof Date && maturity < eff_dt) ||
      (library.valuation_date instanceof Date &&
        maturity < library.valuation_date)
    )
      throw new Error(
...
(function (library) {
  ...
  function get_slice_rate(surface, i_expiry, t_term, imin, imax) {
    ...
    if (imin + 1 === imax) {
      return (
        (sl[imin] * (get_term_at(surface, imax) - t_term)) /
          (get_term_at(surface, imax) - get_term_at(surface, imin)) +
        (sl[imax] * (t_term - get_term_at(surface, imin))) /
          (get_term_at(surface, imax) - get_term_at(surface, imin))
      );
    }
    //binary search and recursion
    imed = Math.ceil((imin + imax) / 2.0);
    if (t_term > get_term_at(surface, imed))
      return get_slice_rate(surface, i_expiry, t_term, imed, imax);
  ...
  library.get_surface_rate = function (surface, t_expiry, t_term, imin, imax) {
    ...
    //extrapolation (constant)
    if (t_expiry < get_expiry_at(surface, imin))
      return get_slice_rate(surface, imin, t_term);
    if (t_expiry > get_expiry_at(surface, imax))
      return get_slice_rate(surface, imax, t_term);
    //interpolation (linear)
    if (imin + 1 === imax) {
      return (
        (get_slice_rate(surface, imin, t_term) *
          (get_expiry_at(surface, imax) - t_expiry)) /
          (get_expiry_at(surface, imax) - get_expiry_at(surface, imin)) +
        (get_slice_rate(surface, imax, t_term) *
          (t_expiry - get_expiry_at(surface, imin))) /
          (get_expiry_at(surface, imax) - get_expiry_at(surface, imin))
      );
    }
    //binary search and recursion
    imed = Math.ceil((imin + imax) / 2.0);
    if (t_expiry > get_expiry_at(surface, imed))
      return library.get_surface_rate(surface, t_expiry, t_term, imed, imax);
...
(function (library) {
  ...
  library.swaption.prototype.present_value = function (
    ...
    var std_dev =
      library.get_surface_rate(vol_surface, t_first_exercise_date, t_term) *
      Math.sqrt(t_first_exercise_date);
    ...
    if (t_first_exercise_date < 0) {
      ...
      res =
        this.swap.phi *
          (this.swap.fixed_rate - fair_rate) *
          library.cndf(this.swap.phi * d1) +
        std_dev * library.ndf(d1);
    }
  ...
  library.create_equivalent_regular_swaption = function (
    ...
    var ttm =
      Math.abs(irr) < 0.00000001
        ? effective_duration_target
        : -Math.log(1 - effective_duration_target * irr) / irr;
...
(function (library) {
  ...
  library.get_safe_date_vector = function (d) {
    //vector of dates when vector of dates, vector of date strings or space sepatated list of date strings is entered. Returns null otherwise
    if (d instanceof Date) return [d];
    var res;
    if (typeof d === "string") {
      res = d.split(/\s+/);
  ...
  function yf_30E360(from, to) {
    return (
      ((to.getFullYear() - from.getFullYear()) * 360 +
        (to.getMonth() - from.getMonth()) * 30 +
        (Math.min(to.getDate(), 30) - Math.min(from.getDate(), 30))) /
      360
    );
  }
  ...
  function yf_actact(from, to) {
    ...
    var res = yto - yfrom - 1;
    res +=
      days_between(from, new Date(yfrom + 1, 0, 1)) /
      (is_leap_year(yfrom) ? 366 : 365);
    res +=
      days_between(new Date(yto, 0, 1), to) / (is_leap_year(yto) ? 366 : 365);
    return res;
  }
...
(function (library) {
  ...
  var normalise_scalar = function (obj) {
  ...
  var normalise_curve = function (obj) {
  ...
  var normalise_surface = function (obj) {
  ...
  var update_vector_length = function (len) {
    ...
    if (len !== stored_params.vector_length)
      throw new Error(
        "vector_pricing: parameters need to have the same length or length one",
      );
  };

  library.store_params = function (params) {
    stored_params = { vector_length: 1, scalars: {}, curves: {}, surfaces: {} };
